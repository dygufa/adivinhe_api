...
fu.route = function (path, handler) {
    Map[path] = function (req, res) {
        // CORS
        var origin = req.headers.origin;
        if (origin) {
            var allowedOrigins = ['https://adivinhe.com', 'https://www.adivinhe.com', 'https://adivinhe.com.br', 'https://www.adivinhe.com.br'];
...
var server = createServer(function (req, res) {
    ...
    res.setCookie = function (key, value) {
        ...
        res.setHeader("Set-Cookie", key.trim() + '=' + value.trim());
    }
    ...
    req._get = qs.parse(url.parse(req.url).query);

    /**
     * Executa ação agendada para url
     */
...
fu.listen = function (port, host) {
    ...
    sys.puts("Servidor iniciado na porta: " + port + ".");
};
